...
public sealed class PaymentFaker
{
    ...
    public string CardLastFour(string cardNumber) => cardNumber[^4..];
    ...
    public ProcessPaymentRequest ValidRequest()
    {
        var (month, year) = FutureExpiry();
        return new ProcessPaymentRequest(CardNumber(), month, year, Currency(), Amount(), Cvv());
    }
}
